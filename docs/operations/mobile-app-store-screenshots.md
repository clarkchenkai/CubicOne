...
1. Creates three temporary T3 base directories and starts a local server for each on an available
   port.
2. Creates CubicOne, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic CubicOne review diff.
...
The Pending rows use the production offline outbox and point at the real CubicOne and React fixture

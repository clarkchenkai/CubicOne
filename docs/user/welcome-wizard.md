...
CubicOne shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened CubicOne directly from a
...
- **T3 Connect** connects computers that are signed in to your account.
  ...
  computer you want to add, then start CubicOne or run `t3 serve` so the
...
If CubicOne cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If CubicOne cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

CubicOne checks each selected computer for Claude Code and Codex. If an agent is
...
CubicOne finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. CubicOne keeps the
...
30 days. You can continue those conversations in CubicOne.

Conversation import is best effort. CubicOne keeps the first user prompt and the
...
conversation so CubicOne does not remove user text. It reads one conversation at

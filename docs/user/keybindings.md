...
`~/.cubicone/userdata/keybindings.json` by default. You can edit this file directly.
...
CubicOne creates the file with its defaults and adds new defaults on later startups.
...
Invalid rules are ignored; if the file cannot be parsed, CubicOne uses defaults.

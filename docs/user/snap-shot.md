...
Shift keys together. CubicOne attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while CubicOne is in front captures CubicOne itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. CubicOne refuses shortcuts
...
terminal contents. If an app is slow to answer, CubicOne attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **CubicOne SnapShots** extension during setup. It is installed
...
shortcut does not fire, check CubicOne under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. CubicOne shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. CubicOne shows the exact binding it will
...
**Other Wayland desktops.** CubicOne uses the desktop's screenshot portal when it can capture the

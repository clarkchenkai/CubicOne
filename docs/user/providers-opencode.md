...
CubicOne requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let CubicOne start OpenCode locally. A password in
provider settings applies to both that server and CubicOne's connection. With no
...
settings. CubicOne uses only that configured password for an external server; it
...
restart before CubicOne can see configuration changes.

...
CubicOne runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for CubicOne to confirm
...
hostname. Only that CubicOne sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. CubicOne's separate Plan mode
...
CubicOne keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. CubicOne also reads `.gemini/skills` and the

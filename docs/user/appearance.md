...
On web and desktop, choose **Create theme** to adjust a palette, or import a CubicOne or VS Code
...
takes precedence. If the server stops publishing the selected theme, CubicOne falls back to its
standard theme.
...
Save a theme exported from CubicOne into `~/.cubicone/userdata/themes/` on the server, or the `themes`
...
Set `appearance` to `light` or `dark` and supply hex colors for `canvas` and `accent`. CubicOne

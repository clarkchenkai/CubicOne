...
CubicOne uses Claude Code's login and configuration. Start with the default provider
...
CubicOne uses the Claude configuration on the connected server.
...
Select the model you want in CubicOne. For an OpenRouter model outside the built-in
...
do not replace the explicit model ID selected in CubicOne. Custom models may have

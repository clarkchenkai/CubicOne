...
function FullDiskAccessStep({
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>Let CubicOne read {source.name}&rsquo;s cookies</DialogTitle>
        <DialogDescription>
          To import cookies from {source.name}, CubicOne needs Full Disk Access. Turn it on in
      ...
      <DialogPanel>
        ...
        {!permission.isReady(["fullDiskAccess"]) ? (
          <p className="mt-3 text-xs text-muted-foreground">
            {stillRequired
              ? "Access is still required. Quit and reopen CubicOne if you just allowed it, then retry the import."
              : "If access doesn't update after you allow it, quit and reopen CubicOne, then retry the import."}
          </p>
        ) : null}
      </DialogPanel>
      <DialogFooter>

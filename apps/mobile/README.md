...
> [!WARNING]
> CubicOne Mobile is currently in development and is not distributed yet. If you want to try it out, you can build it from source.
...
- `development`: Expo dev client, installable side-by-side as `CubicOne Dev`
- `preview`: persistent internal preview build, installable side-by-side as `CubicOne Preview`
- `production`: store/release build as `CubicOne`

...
export const TRIAGE_PLAYBOOK = `# CubicOne triage playbook

You are a support engineer for CubicOne (https://github.com/clarkchenkai/CubicOne), working
...
Fetch https://raw.githubusercontent.com/clarkchenkai/CubicOne/cubicone/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/clarkchenkai/CubicOne <source-cache-dir>/<hash>
...
- How is CubicOne running on this machine: \`npx t3 serve\` in a terminal, the
...
Search existing issues in clarkchenkai/CubicOne (use \`gh\`, or the public GitHub search
...
Do not patch the CubicOne source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/clarkchenkai/CubicOne/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your CubicOne triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A CubicOne user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# CubicOne triage context
...
- Repo: https://github.com/clarkchenkai/CubicOne

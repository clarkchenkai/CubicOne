        ...
        probe: {
          ...
          message:
            openCodeSettings.serverUrl.trim().length > 0
              ? "OpenCode is disabled in CubicOne settings. A server URL is configured."
              : "OpenCode is disabled in CubicOne settings.",
        },
      });
    }
...
> {
  ...
  if (!isExternalServer) {
    ...
    if (!version) {
      return fallback(
        new Error(
          `Unable to determine OpenCode version from \`opencode --version\` output. CubicOne requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
        ),
        null,
      );
    }

...
<header>↑ Drag CubicOne into the list above</header>
<button id="app" draggable="true" aria-label="Drag CubicOne to System Settings, or click to reveal in Finder"><img src="${escapeHtml(icon)}" alt="" draggable="false">CubicOne</button>
</main></body></html>`;
}
...
export class MacPermissionHelper {
  ...
  async show(
    ...
    if (!appIcon) throw new Error("The packaged CubicOne icon is missing.");

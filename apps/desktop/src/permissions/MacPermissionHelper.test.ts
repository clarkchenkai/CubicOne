...
vi.mock("electron", async () => {
  ...
  return {
    app: {
      getPath: () => "/Applications/CubicOne (Nightly).app/Contents/MacOS/CubicOne",
    },
...
describe("macAppBundlePath", () => {
  it("resolves bundles with spaces and refuses non-bundle executables", () => {
    expect(macAppBundlePath("/Applications/CubicOne.app/Contents/MacOS/CubicOne")).toBe(
      "/Applications/CubicOne.app",
    );
    expect(macAppBundlePath("/usr/local/bin/electron")).toBeUndefined();
    expect(macAppBundlePath("/Applications/CubicOne.app/other/MacOS/CubicOne")).toBeUndefined();
  });
});
it("drags the running app bundle only for the helper's own renderer", async () => {
  ...
  expect(mocks.startDrag).toHaveBeenCalledWith({
    file: "/Applications/CubicOne (Nightly).app",
    icon: mocks.createFromPath.mock.results[0]!.value.resize(),
  });
  send("finder");
  expect(mocks.showItemInFolder).toHaveBeenCalledWith("/Applications/CubicOne (Nightly).app");
});
...
it("does not show a helper with a missing packaged icon", async () => {
  ...
  await expect(open()).rejects.toThrow("packaged CubicOne icon is missing");
  expect(windows).toHaveLength(0);
});

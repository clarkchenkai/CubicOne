...
# Invoked by ssh via SSH_ASKPASS when CubicOne re-runs ssh with a cached password
...
printf 'CubicOne ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when CubicOne re-runs\r
...
[Console]::Error.WriteLine("CubicOne ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
